namespace _07
{
    public class Tree
    {
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        private TreeNode GenerateTree(int s, int e)
        {
            if (s > e)
            {
                return null;
            ...
            for (var i = s; i <= e; i++)
            {
                ...
                {
                    node = new TreeNode(_preOrder[_tag++]);
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        private TreeNode GenerateTree2(int s, int e)
        {
            if (s > e)
            {
                return null;
            ...
            for (var i = s; i <= e; i++)
            {
                ...
                {
                    node = new TreeNode(_postOrder[_tag--]);

                    // 递归遍历生成右子树
                    node.right = GenerateTree2(i + 1, e);

                    // 递归遍历生成左子树
                    node.left = GenerateTree2(s, i - 1);
